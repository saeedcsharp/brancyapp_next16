...
- Add component/browser coverage for store coupon loading, `isActive`/`isPrivate` filtering, `nextMaxId` pagination, duplicate prevention, creation validation, API errors, `showInBio` updates, statistics-grid width alignment, and mobile/RTL presentation when a UI test harness is introduced.

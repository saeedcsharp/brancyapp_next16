...
No confirmed runtime bugs remain from the chart navigation issue addressed on 2026-07-20 or the AI-route navbar/sidebar logo issues addressed on 2026-07-25. The market mobile navbar logo mismatch was fixed on 2026-08-22 by aligning route enum values with the actual App Router paths. The `/message/Properties` and `/setting/subAdmin` navbar visibility issues were fixed on 2026-09-19 by aligning desktop and mobile route matching with lowercase pathname normalization; both tabs now also receive the correct active index.

...
`components/wallet/bankCard.tsx` owns the complete bank-card collection view. It renders the shared horizontal `Slider` and `SliderSlide` mapping, sorts the default card to the first real-card position, starts the slider on that card, and preserves normal user navigation. It also owns each card's general-balance status totals, date filter, loading state, and existing card-selection action; balance entries are matched by `cardNumber` and totaled independently for all four `SubInvoiceStatus` values. Its first add-card slide also owns the inline card-number form, API request, validation, notifications, and successful-registration callback; no standalone add-card component remains.

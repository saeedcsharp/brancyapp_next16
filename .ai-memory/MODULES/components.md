...
`components/wallet/settle.tsx` owns settlement-history loading and cursor pagination from `/api/wallet/getInvoices`. It flattens invoice sub-invoices and renders only awaiting, settled, and failed settlement records with the same wallet card structure and shared `IDpurple`, `IDgreen`, and `IDred` status styles used by sub-invoice history.

`components/wallet/modal/invoicePopup.tsx` shows the selected invoice's sub-invoice history. Selecting its `Order` tab calls the parent-provided invoice-detail callback with that invoice ID, then renders `OrderDetailPopup` inline inside the same popup when the full invoice is returned; its Back and Close actions return to the invoice summary, and the parent owns the outer close action.

...
Top-level configuration and deployment files.

## Business Purpose

Controls build, type checking, PWA, Docker, IIS, Sass/PostCSS, package installation, and ignores.
...
`package.json`, `package-lock.json`, `next.config.js`, `tsconfig.json`, `postcss.config.js`, `.gitignore`, `.dockerignore`, `Dockerfile`, `web.config`, `.env`.

...
The `Order` tab requires `RoleAccess(session, PartnerRole.Orders)`. Users without that partner role see `NotAllowedCard`, and order-detail loading is skipped. Order-detail-only popup selectors are scoped to the `OrderDetailPopup` wrapper so they cannot hide the access-denied state.

...
Selecting a bank-card tile opens `components/wallet/subInvoicePopup.tsx`. Its header close action closes the parent modal. `payment.tsx` caches sub-invoice responses by card number, preventing a repeated initial `/api/wallet/getSubInvoices` request when the same popup is reopened, and conditionally unmounts the child before modal close animation so no empty-card request occurs. The popup appends unique later pages through its `nextMaxId` cursor and `useInfiniteScroll`.

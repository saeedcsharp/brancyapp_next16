...
Selecting an invoice opens its sub-invoice history. The `headerChild` in `components/wallet/invoicePopup.tsx` contains SVG order-details and close actions; the former calls `payment.tsx`'s `/api/wallet/getInvoice` request with the selected invoice ID, and the latter closes the parent modal. On success, it opens `components/wallet/orderDetailPopup.tsx`, which retrieves `/api/order/GetFullOrder` using the returned invoice's order ID and user ID, then renders the shared read-only store order-detail content with its own close action; no order accept/reject controls are exposed from the wallet view.

Selecting a bank-card tile opens `components/wallet/subInvoicePopup.tsx`. Its shared `ToggleButton` separates history from settings; settings set the selected card as default through `/api/wallet/setDefaultCard` and its `cardNumber` query. Its header close action closes the parent modal. `payment.tsx` caches sub-invoice responses by card number, preventing a repeated initial `/api/wallet/getSubInvoices` request when the same popup is reopened, and conditionally unmounts the child before modal close animation so no empty-card request occurs. The popup appends unique later pages through its `nextMaxId` cursor and `useInfiniteScroll`.

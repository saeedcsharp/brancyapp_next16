...
`payment.tsx` loads cards through `clientFetchApi` from `/api/wallet/getInstagramerBankCards`, normalizes supported backend response shapes to an array, and passes the data and callbacks to `components/wallet/bankCard.tsx`. `BankCard` owns the horizontal slider, card mapping, and inline add-card form. The page independently retrieves `/api/wallet/getInvoices`, then uses its `nextMaxId` cursor with `useInfiniteScroll` to append unique subsequent invoice pages after the card collection through `components/wallet/invoices.tsx`.

Selecting an invoice opens its sub-invoice history. The `headerChild` in `components/wallet/modal/invoicePopup.tsx` contains SVG order-details and close actions; the former calls `payment.tsx`'s `/api/wallet/getInvoice` request with the selected invoice ID, and the latter closes the parent modal. On success, it opens `components/wallet/modal/orderDetailPopup.tsx`, which retrieves `/api/order/GetFullOrder` using the returned invoice's order ID and user ID, then renders the shared read-only store order-detail content with its own close action; no order accept/reject controls are exposed from the wallet view.

Selecting a bank-card tile opens `components/wallet/modal/subInvoicePopup.tsx`. Its shared `ToggleButton` separates history from settings; settings set the selected card as default through `/api/wallet/setDefaultCard` and its `cardNumber` query. Its header close action closes the parent modal. `payment.tsx` caches sub-invoice responses by card number, preventing a repeated initial `/api/wallet/getSubInvoices` request when the same popup is reopened, and conditionally unmounts the child before modal close animation so no empty-card request occurs. The popup appends unique later pages through its `nextMaxId` cursor and `useInfiniteScroll`.
...
The App Router exposes `/wallet/payment` as the single Instagramer wallet tab. The payment page renders the balance summary and a free horizontal bank-card slider; each `BankCard` item renders its own general-balance status totals and date filter below the card, while card selection still opens the sub-invoice popup. The first slider item is the add-card variant, which owns and expands the registration form inline below its tile and refreshes cards after success. `/wallet`, `/wallet/statistics`, and `/wallet/title` redirect to `/wallet/payment` for backward compatibility.

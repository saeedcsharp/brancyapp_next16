...
- The desktop product list renders its table header inside the same scroll container as the product rows. The header remains sticky at the top, and both header and rows use the same six-column grid: flexible product details, PID, stock, price, last modified, and status.
...
- Store product detail variation price inputs (instance and non-instance) now render each product's `PriceType` currency in the shared `InputBox` unit slot via `specifyPriceType`, while keeping the existing formatted/raw edit flow.
- Store product detail setting inputs (instance and non-instance) render gram, CM, and envelope quantity labels through the shared `InputBox.unit` slot rather than local `subinput` wrappers.
...
- Product-detail media sortable lists in both instance and non-instance editors use a local dnd-kit horizontal-axis modifier, so sortable transforms cannot move items vertically. Pointer dragging starts after an 8px distance threshold to avoid treating ordinary clicks as reorder gestures.

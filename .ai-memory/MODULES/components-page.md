...
The square expansion control is used only when the model exposes exactly one unique Range input for each of the top, right, bottom, and left directions. A model with an incomplete range combination renders each Range input as the standard single-axis slider.

...
`MediaCreator` exposes an image-only suggested-prompt trigger. `imagePromptSuggestions.tsx` owns the suggestion-list content, category filtering, and cursor pagination for `MediaAi/GetImagePrompts`; the AI page parent owns both modal wrappers and selected-prompt state. Selecting a card fills the image prompt field and opens a parent-owned detail modal containing `promptBody` with a clipboard copy action.

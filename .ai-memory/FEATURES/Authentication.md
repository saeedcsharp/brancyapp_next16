...
The Meta direct-login flow guards its verification request so React Strict Mode effect replays cannot call the API twice. Its loading phrases use a deterministic order for SSR and the initial client render, then shuffle after hydration to avoid text mismatches. After successful verification, it waits 10 seconds and opens the `initialSetup` language/theme/calendar flow instead of navigating automatically. Navigation to `/directlogin` occurs only after setup completion.

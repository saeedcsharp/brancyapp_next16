...
The FeatureBox popup provides Working Hours and Terms & Conditions sections through the shared `ToggleButton`. It fetches both sections from `Instagramer/Bio/GetWorkingHours` and `Instagramer/Bio/GetTermsAndCondtions`, shows the shared loader during the initial requests, embeds the existing business-hours editor, and provides an Announcement-style editable Terms component with one text area, character counter, cancel, and save actions. Terms use an `{ str: string }` response/request body and are saved through `Instagramer/Bio/UpdateTermsAndConditions`; the text is limited to 1,500 characters. Working hours are saved as `IBusinessHour[]` through `Instagramer/Bio/UpdateWorkingHours`.

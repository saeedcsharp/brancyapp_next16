import styles from "./settlePopup.module.css";
...
interface SettlePopupProps {
  cardNumber: string;
  onClose: () => void;
}

export default function SettlePopup({ cardNumber, onClose }: SettlePopupProps) {
  ...
  return (
    <div className={styles.container}>
      ...
      <h3 className={styles.title}>{t("Do you want to settle for this card?")}</h3>
      <p className={styles.description}>{t("")}</p>
      <div className={styles.actions}>
        <button className={"cancelButton"} onClick={onClose}>
          {t("close")}
        </button>
        <button className={"saveButton"} onClick={handleSettle}>
          {t("settle")}
        </button>
      </div>
    </div>

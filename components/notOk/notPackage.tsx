...
function NotPackage(props: { removeMask: () => void }) {
  ...
  useEffect(() => {
    if (LoginStatus(session)) props.removeMask();
  }, []);
    ...
    <>
      <div
        ...
        <div className={styles.popupsignup}>
          ...
          <div className="headerandinput" style={{ alignItems: "center" }}>
            <div className="title"> {" package  expired"}</div>

            <div className={`explain translate`}>{"Your package is expired"}</div>
          </div>
        </div>
      </div>
    </>

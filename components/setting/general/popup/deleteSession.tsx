...
import { ISession } from "saeed/models/_AccountInfo/InstagramerAccountInfo";
...
const DeleteSession = (props: {
  ...
  return (
    <>
      <div className={styles.deleteSessionPopup}>
        ...
        <div className="headerandinput" style={{ alignItems: "center" }}>
          ...
          <div className="explain" style={{ textAlign: "center" }}>
            {props.session.createdTime * 1e3 < Date.now() + 86400000
              ? t(LanguageKey.SettingGeneral_cantdeletesession)
              : t(LanguageKey.SettingGeneral_deletesession) + props.session.sessionId}
          </div>
        </div>
        <div className="ButtonContainer">
          ...
          {
            <button
              disabled={props.session.createdTime * 1e3 < Date.now() + 86400000}
              onClick={() => {
                props.handleDeleteSession(props.session.sessionId);
              }}
              className={props.session.createdTime * 1e3 < Date.now() + 86400000 ? "disableButton" : `saveButton`}>

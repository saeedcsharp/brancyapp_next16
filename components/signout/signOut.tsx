...
function SignOut(props: { removeMask: () => void }) {
  ...
  async function sendSessionId() {
    // Always save session ID and set deleteSession to false
  ...
  // Removed checkBox state since we're always saving the session
      ...
      <div
        ...
        <div className={styles.popupsignup}>
          ...
          <div className="headerandinput" style={{ alignItems: "center" }}>
            ...
            <div className={`explain translate`}>{t(LanguageKey.logoutExplain)}</div>
          </div>
          {/* Removed the checkbox UI element */}
          <button onClick={signOut2} className="stopButton">
            {t(LanguageKey.logout)}
          </button>
        </div>
      </div>
    </>

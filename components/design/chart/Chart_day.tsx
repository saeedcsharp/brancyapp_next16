...
const getSmoothPath = (points: Point[]): string => {
  ...
  const fmt = (n: number) => {
    ...
    const rounded = Math.abs(n - Math.round(n)) < 0.0001 ? Math.round(n) : Number(n.toFixed(2));
...
const MultiChart: React.FC<MultiChartProps> = (props) => {
  ...
  // Build day-level entries (one entry per calendar day) instead of months
  const allMonthsData = useMemo(() => {
    if (!props.seriesData || props.seriesData.length === 0) return [];
    const daysMap = new Map<
      string,
      { day: number; month: number; year: number; data: Map<string, { dayList: DayCountUnix[]; totalCount: number }> }
    >();
    props.seriesData.forEach((series) => {
      if (!series.data || series.data.length === 0) return;
      series.data.forEach((monthData) => {
        const list = monthData.dayList || [];
        (list || []).forEach((dayItem) => {
          const key = `${dayItem.year}-${dayItem.month}-${dayItem.day}`;
          if (!daysMap.has(key)) {
            daysMap.set(key, {
              day: dayItem.day,
              month: dayItem.month,
          ...
          const existing = entry.data.get(series.id);
          if (existing) {
            existing.dayList.push(dayItem);
            existing.totalCount += dayItem.count || 0;
          } else {
            entry.data.set(series.id, { dayList: [dayItem], totalCount: dayItem.count || 0 });
          }
        });
      });
    });
  ...
  const hourFormatter = useMemo(() => {
    try {
      return new Intl.DateTimeFormat(localeWithCalendar, { hour: "numeric" });
    } catch {
  ...
  const getMonthYearLabels = useCallback(
    (year: number, month: number, day?: number) => {
      const d = day ? new Date(year, Math.max(0, month - 1), day) : new Date(year, Math.max(0, month - 1), 1);
      return {
        month: `${dayFormatter.format(d)} ${monthFormatter.format(d)}`,
  ...
  const plusCount = useMemo(() => {
    ...
    if (prevSeriesCandidates.length > 0) {
      ...
      let prevTotal = 0;
      for (const ps of prevSeriesCandidates) {
        const entry = currentMonthData.data.get(ps.id) as any;
        if (entry && typeof entry.totalCount === "number") prevTotal += entry.totalCount;
        else if (entry && Array.isArray(entry.dayList))
          prevTotal += entry.dayList.reduce((s: number, it: any) => s + (it.count || 0), 0);
      }
      return totalCount - prevTotal;
    }

    // Fallback: compare with previous day entry in allMonthsData
  ...
  const chartData = useMemo(() => {
    ...
    // Aggregate items into hourly buckets (start of hour timestamps)
    const seriesHourlyMaps: Map<number, number>[] = visibleSeriesData.map((s) => {
      const m = new Map<number, number>();
      for (const it of s.items) {
        const ts = (it.createdTime || 0) * 1000;
        const d = new Date(ts);
        d.setMinutes(0, 0, 0);
        const hourTs = d.getTime();
        m.set(hourTs, (m.get(hourTs) || 0) + (it.count || 0));
      }
      return m;
    });
    const tsSet = new Set<number>();
    for (let i = 0; i < seriesHourlyMaps.length; i++) {
      seriesHourlyMaps[i].forEach((_, k) => tsSet.add(k));
    }
    // Determine end hour (use latest data timestamp or now) and build last 24 hours timeline
    let latest = tsSet.size > 0 ? Math.max(...Array.from(tsSet)) : Date.now();
    const endHour = new Date(latest);
    endHour.setMinutes(0, 0, 0);
    latest = endHour.getTime();
    const hours: number[] = [];
    for (let i = 23; i >= 0; i--) {
      hours.push(latest - i * 3600 * 1000);
    }
    const allTimestamps = hours;
    ...
    for (let si = 0; si < seriesHourlyMaps.length; si++) {
      const m = seriesHourlyMaps[si];
    ...
    const leftPadding = Math.max(0, Math.ceil(measuredLabelWidth) + 26);
    ...
    for (let i = 0; i <= xSteps; i++) {
      ...
      xLabels.push({
        x,
        timestamp,
        label: state.showAll ? monthFormatter.format(new Date(timestamp)) : hourFormatter.format(new Date(timestamp)),
      });
    }
    ...
    for (let seriesIndex = visibleSeriesData.length - 1; seriesIndex >= 0; seriesIndex--) {
      ...
      const dataMap = seriesHourlyMaps[seriesIndex];
      const points: Point[] = [];
      for (let ti = 0; ti < allTimestamps.length; ti++) {
        const timestamp = allTimestamps[ti];
        let stackedY = 0;
        for (let j = seriesIndex; j < seriesHourlyMaps.length; j++) {
          stackedY += seriesHourlyMaps[j].get(timestamp) || 0;
        }
                ...
                <>
                  ...
                  <g fill="var(--text-h2)" fontWeight="400">
                    {chartData.xLabels.map((label, i) => {
                      ...
                      // rotate labels to avoid overlap (oblique)
                      return (
                        <text
                          key={`x-label-${i}`}
                          x={x}
                  ...
                  <g>
                    ...
                    {chartData.seriesPaths.map((series: any, idx: number) => (
                      <g key={`series-${idx}`}>
                        {/* Area fill */}
                        <path d={series.areaPath} fill={`url(#gradient-${props.id}-${idx})`} />
                        {/* Line stroke */}
                        <path d={series.linePath} fill="none" stroke={series.color} strokeWidth="1" />
                  ...
                  {state.hoveredPoint && (
                    <line
                      x1={state.hoveredPoint.x}
                      y1={chartData.padding.top}
                      x2={state.hoveredPoint.x}
            ...
            {chartData && (
              <div
                ...
                {chartData.yGridLines.map((line: any, i: number) => (
                  <div
                    key={`hgrid-${i}`}
                    className={multiStyles.gridLine}
                    style={{
                ...
                {chartData.xGridLines.map((line: any, i: number) => (
                  <div
                    key={`vgrid-${i}`}
                    className={multiStyles.gridLineV}
                    style={{
                ...
                {/* average line + label */}
                {(props.showAverage ?? true) && typeof chartData.averageY === "number" && (
                  <>
                    <div
                    ...
                    <div
                      ...
                      {chartData.averageValue % 1 === 0
                        ? Math.round(chartData.averageValue).toLocaleString()
                        : chartData.averageValue.toLocaleString(undefined, {
            ...
            {state.hoveredPoint && state.tooltipPos && chartData && (
              <div
                ...
                <div className={multiStyles.tooltipHeader}>
                  {hourFormatter.format(new Date(state.hoveredPoint!.timestamp))} —{" "}
              ...
              <div className="headerandinput" style={{ alignItems: "center" }}>
                ...
                <span className={`${multiStyles.totalcounter} translate`} style={{ color: "var(--color-dark-blue)" }}>
                  ...
                  {(() => {
                    ...
                    return (
                      <strong>
                        {dayFormatter.format(prev)} {monthFormatter.format(prev)}
                      </strong>
                    );
                  })()}
                </span>

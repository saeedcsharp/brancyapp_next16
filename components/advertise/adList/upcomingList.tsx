    ...
    <section className="tooBigCard" style={{ gridRowEnd: isHidden ? "span 10" : "span 82" }}>
      <div className="frameParent" onClick={handleCircleClick}>
        <div className="headerChild">
          ...
          <h2 className="Title">{t(LanguageKey.advertiseadlist_AwaitingPublication)}</h2>
        </div>
      </div>
        ...
        <Slider className={styles.section} itemsPerSlide={5}>
          {props.data.map((u) => (
            <SliderSlide key={u.adsId}>
              <div onClick={() => props.handleShowDetailAds(u.adsId)} className={styles.list}>
                <div className={styles.up}>
                  <div className={styles.timer}>
                    <CounterDown2
                      upingTime={u.upingTime}
                      isDead={() => props.handleRemoveUpingItem(u.adsId.toString())}
                      classNamewrapper="countdownWrapperPost"
                      colorSvg="var(--color-dark-blue)"
                      colorTimeTitle="var(--color-dark-blue)"

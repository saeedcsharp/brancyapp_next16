...
import { IAdMonth } from "saeed/models/advertise/statistics";
  ...
  return (
    <section className={styles.twomonth} style={gridSpan}>
      <div onClick={toggle} className="headerChild" title="↕ Resize the Card">
        ...
        <h2 className="Title">{t(LanguageKey.advertisestatistics_lasttwomonth)}</h2>
      </div>
      {hidePage && (
              ...
              <div className={styles.totalmonth}>
                <div className={styles.month}>
                  ...
                  <div className={styles.monthlabel}>
                    {t(LanguageKey.advertisestatistics_lastmonth)}(<strong>{NumToMonth(props.data[0].month)}</strong>)
                  </div>
                </div>
                <div className={styles.month}>
                  <div
                  ...
                  <div className={styles.monthlabel}>
                    {t(LanguageKey.advertisestatistics_thismonth)} (<strong>{NumToMonth(props.data[1].month)}</strong>)
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className={styles.headercontent}>
                <div className={styles.summery} style={{ backgroundColor: "var(--color-light-blue30)" }}>
                  ...
                  <div className={styles.summerytype} style={{ color: "var(--color-light-blue60)" }}>
                    {t(LanguageKey.advertisestatistics_ads)}
                  </div>
                </div>
                ...
                {calculatAdsRate() !== 0 && (
                  <div className={styles.summery} style={{ padding: "0px" }}>
                    <img
                      ...
                      alt="rate"
                      src={calculatAdsRate() < 0 ? "/decrease.svg" : "/increase.svg"}
                    />
                    <div
                      className={styles.percentage}
                      style={{
                        color: calculatAdsRate() < 0 ? "var(--color-light-red)" : "var(--color-light-green)",
                      }}>
                      <>
                        {calculatAdsRate()} {t(LanguageKey.advertisestatistics_fromlast)}
                      </>
                    </div>
                  </div>
                )}
                ...
                {calculatIncomeRate() !== 0 && (
                  <div className={styles.summery} style={{ padding: "0px" }}>
                    ...
                    <div
                      ...
                      <>
                        {calculatIncomeRate()} {t(LanguageKey.advertisestatistics_fromlast)}
                      </>
                    </div>
                  </div>
                )}

...
const VideoAndMusic = (props: { removeMask: () => void }) => {
  ...
  const saveCondition = useMemo(() => {
    return !(
      (updateYoutube.isActive && !updateYoutube.id && !updateYoutube.username && !channelSearch.searchYoutubePage) ||
      (updateAparat.isActive && !updateAparat.id && !updateAparat.username && !channelSearch.searchAparatPage) ||
      (updateTwitch.isActive && !updateTwitch.id && !updateTwitch.username && !channelSearch.searchTwitchPage)
    );
  }, [updateYoutube, updateAparat, updateTwitch, channelSearch]);
      ...
      if (query.length > 0) {
        timeoutRef.current = setTimeout(() => {
          if (query && !channelBox.peopleLocked) {
            if (selectedEmbed === "searchYoutubePage" && !query.startsWith("uc") && !query.startsWith("UC")) {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (channelSearch.activeTwitch && (updateTwitch.id || updateTwitch.username || channelSearch.searchTwitchPage)) {
      try {
  ...
  return (
    <>
      ...
      <Head>
        ...
        <title>Bran.cy ▸ Edit last video</title>
          ...
          {selectedEmbed === "searchYoutubePage" && (
            <div className={styles.all}>
              ...
              <div className={`headerandinput ${!channelSearch.activeYoutube && "fadeDiv"}`}>
                ...
                {channelSearch &&
                  channelSearch.searchYoutubePage &&
                  typeof channelSearch.searchYoutubePage === "string" &&
                  channelSearch.searchYoutubePage.length > 0 &&
                  channelBox.showAddPeapleBox && (
                ...
                {channelSearch.youTubeThumbnailUrl && channelSearch.youTubeThumbnailUrl.length > 0 && (
                  <img
                    loading="lazy"
                    decoding="async"
                    alt="thumbnail"
          ...
          {selectedEmbed === "searchAparatPage" && (
            <div className={styles.all}>
              ...
              <div className={`headerandinput ${!channelSearch.activeAparat && "fadeDiv"}`}>
                ...
                {channelSearch &&
                  channelSearch.searchAparatPage &&
                  typeof channelSearch.searchAparatPage === "string" &&
                  channelSearch.searchAparatPage.length > 0 &&
                  channelBox.showAddPeapleBox && (
                    <div className={styles.resultSearchmention}>
                      {!channelBox.notFound &&
                        !channelBox.loading &&
                ...
                {channelSearch.aparatThumbnailUrl && channelSearch.aparatThumbnailUrl.length > 0 && (
                  <img
                    loading="lazy"
                    decoding="async"
                    alt="thumbnail"
          ...
          {selectedEmbed === "searchTwitchPage" && (
            <div className={`${styles.all}`}>
              ...
              <div className={`headerandinput ${!channelSearch.activeTwitch && "fadeDiv"}`}>
                ...
                {channelSearch &&
                  channelSearch.searchTwitchPage &&
                  typeof channelSearch.searchTwitchPage === "string" &&
                  channelSearch.searchTwitchPage.length > 0 &&
                  channelBox.showAddPeapleBox && (
                    <div className={styles.resultSearchmention}>
                      {!channelBox.notFound &&
                        !channelBox.loading &&
                ...
                {channelSearch.twitchThumbnailUrl && channelSearch.twitchThumbnailUrl.length > 0 && (
                  <img
                    loading="lazy"
                    decoding="async"
                    alt="thumbnail"

...
import { Dispatch, KeyboardEvent, ReactNode, SetStateAction, useEffect, useMemo, useState } from "react";
...
const IngageInfo = (props: {
  ...
  const firstLoginAt = useMemo(() => {
    const createdTime = session?.user.createdTime;
    return typeof createdTime === "number" && Number.isFinite(createdTime) && createdTime > 0
      ? createdTime * 1000
      : null;
  }, [session?.user.createdTime]);

  useEffect(() => {
    let isActive = true;
    setAiFeatureCount(null);
    if (!session) return () => undefined;

    getTotalFeatureCount(session, PsgFeatureType.AI).then((count) => {
      if (isActive) setAiFeatureCount(count);
    });
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setCurrentTime(Date.now()), firstLoginAt ? 1000 : 60000);
    return () => window.clearInterval(timer);
  }, [firstLoginAt, session]);
  ...
  const statusMap: StatusItem[] = useMemo(
    () => [
      {
        key: "firstLogin",
        priority: 1,
  ...
  const handleSlideKeyDown = (
    ...
    if (event.key === "ArrowRight" || event.key === "ArrowDown") {
      ...
      setSlide((currentIndex) => (currentIndex + 1) % slideCount);
    } else if (event.key === "ArrowLeft" || event.key === "ArrowUp") {
      event.preventDefault();
      setSlide((currentIndex) => (currentIndex - 1 + slideCount) % slideCount);
  ...
  const upgradeSlides = useMemo(
    ...
    [aiFeatureCount, subscriptionRemainingDays, t],
  );
  const statisticsSlides = useMemo(
    () => [
            ...
            {props.activeStories.length > 0 ? (
              props.activeStories.map((story) => (
                <Link
                  ...
                  aria-label={`${t(LanguageKey.pageStatistics_stories)} ${story.storyId}`}
      ...
      },
      {
        ...
        value:
          props.data?.items.find((x) => x.topTileType === TopTileType.Reach)?.value !== undefined
    ...
    [props.activeStories, props.data, t],
  );
      ...
      {!loadingStatus && props.data && (
        <>
          ...
          <section
            ...
            role="group"
            tabIndex={0}
            aria-label={t(LanguageKey.SmartPageAnalysis)}
          ...
          <section className={styles.totaltile}>
            ...
            <div key={`statistics-${activeStatisticsSlide}`} className={styles.slideshowcontent} aria-live="polite">
              {selectedStatisticsSlide.icon}
              <div className={styles.slidecontentcontainer}>
                <div
                  className={styles.pagination}
                  onClick={(event) => event.stopPropagation()}
                  onKeyDown={(event) => handleSlideKeyDown(event, statisticsSlides.length, setActiveStatisticsSlide)}>
          ...
          <section
            ...
            role="group"
            tabIndex={0}
            aria-label={t(LanguageKey.upgradeyouraccount)}
            ...
            <div key={`upgrade-${activeUpgradeSlide}`} className={styles.slideshowcontent} aria-live="polite">
              {selectedUpgradeSlide.icon}
              <div className={styles.slidecontentcontainer}>
                <div
                  className={styles.pagination}
                  onClick={(event) => event.stopPropagation()}
                  onKeyDown={(event) => handleSlideKeyDown(event, upgradeSlides.length, setActiveUpgradeSlide)}>
          ...
          <section className={styles.totaltile} style={{ cursor: "pointer" }}>

...
import OrderDetailWithParcelInfo from "../../../../components/userPanel/orders/popups/OrderDetailWithParcelInfo";
import TicketTitle from "../../../../components/userPanel/orders/popups/ticketSubject";
...
import { LogisticType, OrderStep } from "../../../../models/store/enum";
...
const Sent = () => {
  ...
  const handleRowClick = (orderId: string, instagramerId: number, trackingId: string | null) => {
  ...
  async function fetchData() {
    try {
      const res = await clientFetchApi<boolean, IOrderByStatus>("/api/order/GetOrdersByStatus", { methodType: MethodType.get, session: session, data: null, queries: [{ key: "status", value: OrderStep.InShipping.toString() }], onUploadProgress: undefined });
      if (res.succeeded) {
  ...
  function handleManageOrderBySocket(order: IOrderPushNotifExtended) {
    if (
      ...
      console.log("Order initialized in sent in user", order.ShortOrder);
      setOrders((prevOrders) => ({
        ...prevOrders,
  ...
  return (
    session?.user.currentIndex === -1 && (
      <>
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {orders.items.map((order, index) => (
                  <tr
                    onClick={() => handleRowClick(order.id, order.shortShop!.instagramerId, order.trackingId)}
                    key={index}
        ...
        {orderDetailId && (
          <OrderDetailWithParcelInfo removeMask={() => setOrderDetailId(null)} orderDetail={orderDetailId!} />
        )}

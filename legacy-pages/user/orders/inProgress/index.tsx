...
const InProgress = () => {
  ...
  async function handleRejectOrder(orderIds: Set<string>) {
    ...
    const results = await Promise.all(
      Array.from(orderIds).map((orderId) =>
        clientFetchApi<boolean, boolean>("/api/userorder/rejectOrder", {
  ...
  function handleManageOrderBySocket(order: IOrderPushNotifExtended) {
    if (
      ...
      return;
    } else if (order.Order.NewStatus === OrderStep.Paid || order.Order.NewStatus === OrderStep.InstagramerAccepted) {
      ...
      console.log("Order initialized in inprogress in user", order.Order);
      setOrders((prevOrders) => ({
        ...prevOrders,

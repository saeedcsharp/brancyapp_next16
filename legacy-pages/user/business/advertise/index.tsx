  ...
  return (
    <div
      ...
      <div className={styles.cardTop}>
        <span className={`${styles.badge} ${styles.badgeAdvertise}`}>Advertise</span>
      </div>
...
export default function BusinessAdvertise() {
  ...
  useEffect(() => {
    ...
    clientFetchApi<boolean, IBusinessResponse>("/api/business/search", {
      ...
      session,
      data: undefined,
      queries: [{ key: "businessType", value: BusinessType.Advertise.toString() }],
  ...
  const fetchMore = useCallback(async (): Promise<IBusiness[]> => {
    ...
    const res = await clientFetchApi<boolean, IBusinessResponse>("/api/business/search", {
      ...
      data: undefined,
      queries: [
        { key: "businessType", value: BusinessType.Advertise.toString() },
  ...
  return (
    session?.user.currentIndex === -1 && (
      <>
        <Head>
          <title>Brancy ▸ Business › Advertise</title>
        ...
        <div className={styles.container} ref={containerRef}>
          ...
          {loading && items.length === 0 ? (
            ...
            <p className={styles.empty}>No advertisers found.</p>
          ) : (
            <>

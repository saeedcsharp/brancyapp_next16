...
const profilePopupReducer = (state: ProfilePopupState, action: ProfilePopupAction): ProfilePopupState => {
  switch (action.type) {
    case "SHOW_POPUP":
      return {
        showPopup: true,
...
const ShowPost = () => {
  ...
  const [profilePopupState, profilePopupDispatch] = useReducer(profilePopupReducer, {
    showPopup: false,
    popupImage: "",
    popupUsername: "",
  });
  ...
  const divArray = useMemo(() => {
    return Array.from({ length: 10 - detailPost.children.length }, (_, index) => (
  ...
  const buildMultiChartProps = useCallback(
    (sfIndex: number) => {
      if (!insight || !insight.superFigures || !insight.superFigures[sfIndex]) return null;
  ...
  function handlePostPictureClick(url: string, childrenMediaType: number, childrenUrl: string) {
    ...
    async (mediaId: string, nextMaxId: string | null) => {
      ...
      try {
        ...
        if (error instanceof Error) {
          ...
          notify(ResponseType.Unexpected, NotifType.Error, "An unexpected error occurred");
        }
      } finally {
      ...
      try {
        ...
        if (res.succeeded) {
          ...
          if (res.value.commentMedia) {
            setComments((prev) => ({
              ...prev!,
              automaticCommentReply: res.value.commentMedia!.automaticCommentReply,
    ...
    query.postid && (
      <>
                    ...
                    <div className={styles.container}>
                      <div className="headerparent">
                        ...
                        {(() => {
                          ...
                          return (
                            <div className="date translate" title="ℹ️ post publish date">
                      ...
                      {detailPost.isDeleted && (
                        <div className="headerparent">
                          <div className="title">{t(LanguageKey.DeletedPost)}</div>
                          <div className="date translate" title="ℹ️ post deleted date">
                            {detailPost.deleteTimeUnix ? (
                              ...
                              <span className="day">{t(LanguageKey.Unavailable)}</span>
                            )}
                          </div>
                        </div>
                      )}
                      <div className="headerandinput" style={{ position: "relative" }}>
                        {!isPlayingInlineVideo || detailPost.mediaType !== MediaType.Video ? (
                          <>
                            <img
                              onClick={() => {
                                ...
                                if (detailPost.mediaType === MediaType.Image)
                                  mediaModal.openImage(basePictureUrl + detailPost.mediaUrl);
                                else if (detailPost.mediaType === MediaType.Video) {
                              ...
                              src={basePictureUrl + detailPost.thumbnailMediaUrl}
                              style={{
                                cursor: detailPost.mediaType === MediaType.Video ? "pointer" : "default",
                              }}
                            />
                            {detailPost.mediaType === MediaType.Video && !isPlayingInlineVideo && (
                              <div
                                onClick={() => {
                                  if (detailPost.canDownload) {
                                    setIsPlayingInlineVideo(true);
                          ...
                          <>
                            ...
                            <video
                              ...
                              onPause={(e) => {
                                if (e.currentTarget.currentTime === 0 || e.currentTarget.ended) {
                                  setIsPlayingInlineVideo(false);
                                  setIsVideoLoading(false);
                                }
                              }}>
                              <source src={basePictureUrl + detailPost.mediaUrl} type="video/mp4" />
                          ...
                          <div className={styles.postpreview}>
                            {detailPost.children.map((v, i) => (
                              <img
                                ...
                                onClick={() => handlePostPictureClick(v.thumbnailMediaUrl, v.mediaType, v.mediaUrl)}
                                style={{
                                  boxShadow:
                      ...
                      <div className="headerparent">
                        <div className={styles.postdetail}>
                          <img width="20px" height="20px" title="ℹ️ like count" alt="like" src={"/icon-like.svg"} />
                        ...
                        <div className={styles.postdetail}>
                          ...
                          />
                          <span>{detailPost.commentCount.toLocaleString()}</span>
                        </div>
                        <div className={detailPost.saveCount > 0 ? styles.postdetail : `${styles.postdetail} fadeDiv`}>
                          <img title="ℹ️ save count" width="20px" height="20px" alt="save" src={"/icon-save.svg"} />
                          <span>{detailPost.saveCount > 0 ? detailPost.saveCount.toLocaleString() : "--"}</span>
                        </div>
                        <div className={detailPost.shareCount > 0 ? styles.postdetail : `${styles.postdetail} fadeDiv`}>
                          <img title="ℹ️ Send count" width="20px" height="20px" alt="send" src={"/icon-send.svg"} />
                          <span>{detailPost.shareCount > 0 ? detailPost.shareCount.toLocaleString() : "--"}</span>
                        </div>
                        <div className={detailPost.viewCount > 0 ? styles.postdetail : `${styles.postdetail} fadeDiv`}>
                          <img title="ℹ️ Reach count" width="20px" height="20px" alt="view" src={"/icon-view.svg"} />
                          <span>{detailPost.viewCount > 0 ? detailPost.viewCount.toLocaleString() : "--"}</span>
                        </div>
                      </div>
                    </div>
                    <div className={styles.container}>
                      {session.user.commentPermission && (
                        <>
                          <div className="headerandinput">
                            <div className="headerparent">
                              <div className="title2">{t(LanguageKey.pageToolspopup_TurnoffCommenting)}</div>
                            ...
                            <div className="explain">{t(LanguageKey.pageToolspopup_TurnoffCommentingExplain)}</div>
                          </div>
                          <div className="headerandinput">
                            <div className="headerparent">
                      ...
                      <div className="headerandinput">
                        <div className="headerparent">
                          ...
                          <div className={`counter translate`}>
                            ...
                            (<strong>{captionLength.toLocaleString()}</strong> / <strong>2,200</strong>)
                          </div>
                        </div>
                        <div className={`${styles.captionfiled} ${isRTL(detailPost.caption) ? "rtl" : "ltr"}`}>
                          {detailPost.caption}
                        </div>
                      </div>
                      <div className="headerandinput">
                        <div className="headerparent">
                          <div className="title">{t(LanguageKey.pageTools_hashtagList)}</div>
                          <div className={`counter translate`}>
                            <div className={styles.icon}>
                              <img className={styles.hashtagicon} alt="Hashtag Icon" src={"/icon-hashtag.svg"} />
                            </div>
                            (<strong>{detailPost.hashtags ? detailPost.hashtags.length : 0}</strong>/<strong>30</strong>
                            )
                          </div>
                        </div>
                        <div className="explain">{t(LanguageKey.pageToolspopup_TopHashtagexplain)}</div>
                        <div className={styles.tophashtagfield}>
                          {detailPost.hashtags?.map((u, j) => (
                            <div key={j} className={styles.tagHashtag} title="ℹ️ Total hashtag usage in all posts">
                              <img className={styles.hashtagicon} alt="Hashtag Icon" src={"/icon-hashtag.svg"} />
                              {u.hashtag}
                      ...
                      <div className={styles.container}>
                        <div className="headerandinput">
                          <div className="headerparent">
                            <div className="title" style={{ gap: "var(--gap-5)" }}>
                              ...
                              <span
                                ...
                                {t(LanguageKey.new)} (<strong>{detailPost.newCommentCount.toLocaleString()}</strong>)
                              </span>
                            </div>
                            <div className="headerparent" style={{ width: "70px" }}>
                              <Tooltip
                              ...
                              <Tooltip tooltipValue={t(LanguageKey.navbar_Comments)} position="bottom" onClick={true}>
                                <img
                                  onClick={() => {
                                    if (comments?.comments.length === 0) return;
                                    router.push(`/message/comments?mediaId=${detailPost.pk}&&commentType=post`);
                                  }}
                          ...
                          <div className={styles.commentbox}>
                            ...
                            />
                            {!searchState.searchMode && comments!.comments.length === 0 && (
                              ...
                              <Slider
                                ...
                                {comments!.comments.map((u, i) => (
                                  <div key={u.id || i} className={styles.comment}>
                                    ...
                                    <div className={styles.commentdetail}>
                                      <div className="headerandinput" style={{ gap: "2px" }}>
                                        ...
                                        <div className="explain">
                                          ...
                                          • {formatTimeAgo(u.createdTime / 1000)}
                                        </div>
                                      </div>
                                      <div
                                      ...
                                      {u.replys!.length > 0 && (
                                        <div
                                          ...
                                          {t(LanguageKey.viewreplies)} <strong>{u.replys?.length}</strong>
                                        </div>
                                      )}
                                    </div>
                                    {profilePopupState.showPopup && (
                                      <>
                                        <div className="dialogBg" onClick={handleClosePopup} />
                                    ...
                                    {showPopupComment !== "" && (
                                      <>
                                        <div className="dialogBg" onClick={() => togglePopup("")} />
                                        <div className={styles.popupContent}>
                                          <div className="headerparent">
                                          ...
                                          <div className={styles.commentall}>
                                            {comments!.comments
                                              .find((x) => x.id === showPopupComment)!
                                              .replys!.map((u) => {
                                                return (
                                                  <div key={u.id} className={styles.comment}>
                                                    <img
                                                      className="instagramimage"
                                                      style={{
                                                        height: "40px",
                                                      ...
                                                      title="◰ resize the picture"
                                                      onClick={() =>
                                                        handleImageClick(basePictureUrl + u.profileUrl, u.username)
                                                      }
                                                      alt="instagram profile picture"
                                                      src={basePictureUrl + u.profileUrl}
                                                      onError={(e) => {
                                ...
                                {!searchState.loading && !searchState.noresult && (
                                  <Slider
                                    ...
                                    {searchComments!.comments.map((u, i) => (
                                      <div key={u.id || i} className={`${styles.comment} translate`}>
                                        <img
                                          className="instagramimage"
                                          style={{
                                            height: "40px",
                                        ...
                                        {showPopupComment && (
                                          <>
                                            <div className="dialogBg" onClick={() => togglePopup("")} />
                                            <div className={`${styles.popupContent} translate`}>
                                              <div className="headerparent">
                                                {t(LanguageKey.allRepliedcomments)}
                                                <img
                                                  title="Close popup"
                                                  className="closepopup"
                                                  alt="close button"
                        ...
                        <div className={`${styles.container} translate`}>
                          ...
                          <div className="headerandinput">
                            ...
                            <div className={styles.Tableheader}>
                              ...
                              <div>
                                {(
                                  (detailPost.commentCount > 0 ? detailPost.commentCount : 0) +
                                  (detailPost.saveCount > 0 ? detailPost.saveCount : 0) +
                                  (detailPost.shareCount > 0 ? detailPost.shareCount : 0) +
                                  (detailPost.likeCount > 0 ? detailPost.likeCount : 0)
                                ).toLocaleString()}
                              </div>
                            </div>
                            <div className={styles.row}>
                              <div className={styles.rowheader}>Like</div>
                              {(detailPost.likeCount > 0 ? detailPost.likeCount : "--").toLocaleString()}
                            </div>
                            <div className={styles.row}>
                              <div className={styles.rowheader}>Comment</div>
                              {(detailPost.commentCount > 0 ? detailPost.commentCount : "--").toLocaleString()}
                            </div>
                            <div className={styles.row}>
                              <div className={styles.rowheader}>Save</div>
                              {(detailPost.saveCount > 0 ? detailPost.saveCount : "--").toLocaleString()}
                            </div>
                            <div className={styles.row}>
                              <div className={styles.rowheader}>Share</div>
                              {(detailPost.shareCount > 0 ? detailPost.shareCount : "--").toLocaleString()}
                            </div>
                            <div className={styles.row}>
                              <div className={styles.rowheader}>Reels Skip Rate </div>
                              {detailPost.reelsSkipRate ? detailPost.reelsSkipRate.toLocaleString() + "%" : "--"}
                            </div>
                          </div>
                          {
                            <>
                          ...
                          }
                          {
                            <div className="headerandinput">
                              <div className="headerparent">
                                ...
                                <div className="title" style={{ paddingInlineEnd: "20px" }}>
                                  {(detailPost.reachCount >= 0 ? detailPost.reachCount : "--").toLocaleString()}
                                </div>
                              </div>
                              <div className={styles.Tableheader}>
                              ...
                              <div className={styles.row}>
                                <div className={styles.rowheader}>Profile Visits</div>
                                {(insight.profileActivityInfo && insight.profileActivityInfo.profileVisits > 0
                              ...
                              <div className={styles.row}>
                                ...
                                {(insight.profileActivityInfo && insight.profileActivityInfo.follows > 0
                                  ? insight.profileActivityInfo.follows
                                  : "--"
                                ).toLocaleString()}
                              </div>
                              <div className={styles.row}>
                                <div className={styles.rowheader}>BioLink Button Taps</div>
                                {(insight.profileActivityInfo && insight.profileActivityInfo.externalLinkTaps > 0
                                  ? insight.profileActivityInfo.externalLinkTaps
                                  : "--"
                                ).toLocaleString()}
                              </div>
                              <div className={styles.row}>
                                <div className={styles.rowheader}>Bussiness Address Taps</div>
                                {(insight.profileActivityInfo && insight.profileActivityInfo.bussinessAddressTaps > 0
                                  ? insight.profileActivityInfo.bussinessAddressTaps
                                  : "--"
                                ).toLocaleString()}
                              </div>
                              <div className={`headerandinput fadeDiv`}>
                                <div className={styles.row}>
                                  <div className={styles.rowheader}>Call Button Taps</div>
                                  {(insight.profileActivityInfo && insight.profileActivityInfo.callButtonTaps > 0
                                    ? insight.profileActivityInfo.callButtonTaps
                                    : "--"
                                  ).toLocaleString()}
                                </div>
                                <div className={styles.row}>
                                  <div className={styles.rowheader}>Email Button Taps</div>
                                  {(insight.profileActivityInfo && insight.profileActivityInfo.emailButtonTaps > 0
                        ...
                        <div className={styles.row}>
                          ...
                          <div className="headerandinput">
                            <div className={styles.Tableheader}>
                              ...
                              <div className={styles.watchtimeheader}>HH:MM:SS</div>
                            </div>
                            <div className={styles.row}>
                              <div className={styles.rowheader}>Video Average Time</div>
                              <div className={styles.watchtime}>
                                {detailPost.videoViewAverageTime && detailPost.videoViewAverageTime > 0
                                  ? (() => {
                            ...
                            <div className={styles.row}>
                              <div className={styles.rowheader}>Video Total Time</div>
                              <div className={styles.watchtime}>
                                {detailPost.videoViewTotalTime && detailPost.videoViewTotalTime > 0
                                  ? (() => {
                                      const { hours, minutes, seconds } = convertMillisecondsToTime(
                          ...
                          ({insight.superFigures && insight.superFigures.length > 0}
                          )&&
                        </div>
                            ...
                            <div className="title">
                              {insight.superFigures[0] &&
                                (() => {
                                  ...
                                  return propsMulti
                                    ? (propsMulti.objectNavigators[0].title ?? "Super Figures")
                                    : "Super Figures";
                                })()}
                            </div>
                            {insight.superFigures[0] &&
        ...
        <Modal
          ...
          <FollowersNonFollowers removeMask={() => setShowFollowersNonFollowers(false)} />
        </Modal>
        <Modal
